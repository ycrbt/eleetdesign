...
export default function Whiteboard({ onExit }: WhiteboardProps) {
  ...
  function endpoint(id: string, side: "source" | "target"): Point | null {
    ...
    const position = id === draggedNodeId && dragPosition ? dragPosition : node.position;
    return side === "source" ? { x: position.x + node.size.width, y: position.y + node.size.height / 2 } : { x: position.x, y: position.y + node.size.height / 2 };
  }
  ...
  return (
    <div ref={boardRef} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} onWheel={(e) => { e.preventDefault(); zoomAt(e.clientX, e.clientY, zoom * (e.deltaY < 0 ? 1.1 : 0.9)); }} className={`relative h-screen w-screen touch-none select-none overflow-hidden bg-slate-50 ${isPanning ? "cursor-grabbing" : "cursor-grab"}`}>
      ...
      {draggedNode&&dragPosition&&<div className="pointer-events-none absolute left-0 top-0 z-[100] origin-top-left" style={{transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}><InfrastructureNodeView node={new InfrastructureNode(draggedNode.componentType,dragPosition)} isDragging pickupScale={0.9} load={sourceConnection?.targetId===draggedNode.id?REQUEST_RATE:0}/></div>}
